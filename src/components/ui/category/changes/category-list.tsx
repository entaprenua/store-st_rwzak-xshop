import { splitProps, createEffect, createResource, type JSX, Match, Switch, createMemo, createContext, useContext, type Accessor, mergeProps } from "solid-js"
...
const CategorySubcategories = (props: CategorySubcategoriesProps) => {
  ...
  const forRoot = () => mode() === "root"
  const forTree = () => mode() === "tree"
  ...
  return (
    <Switch>
      <Match when={forRoot()}>
        <Collection
          queryFn={queryFn}
          queryKey={["categories", "root_children", resolvedStoreId(), parentCategoryId()]}
        >
          {local.children}
        </Collection>
      </Match>
      <Match when={forTree() && hasSubcategories()}>
        <Collection
          data={subcategories()}
          queryKey={["categories", "tree_children", resolvedStoreId(), parentCategoryId()]}
        >
          {local.children}
        </Collection>
      </Match>
...
export { CategoryList, CategoryListView, CategorySubcategories, DefaultCategoryListLoading }
